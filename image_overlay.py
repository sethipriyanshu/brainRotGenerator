...
def _make_caption_image(text, video_width):
    """Render caption text to an RGBA image using PIL with outline (no background). Returns numpy array."""
    try:
    ...
    for w in words:
        current.append(w)
        if len(" ".join(current)) > CAPTION_MAX_CHARS_PER_LINE:
            if len(current) > 1:
    ...
    # Try bold/impact style fonts first for that meme look
    font = None
    font_paths = [
    ...
    for path in font_paths:
        if os.path.isfile(path):
            try:
                font = ImageFont.truetype(path, CAPTION_FONT_SIZE)
                break
            except Exception:
                pass
    if font is None:
    ...
    try:
        bbox = dummy_draw.multiline_textbbox((0, 0), line_text, font=font)
        tw = bbox[2] - bbox[0]
        th = bbox[3] - bbox[1]
    except Exception:
        tw, th = 300, 50

    pad = CAPTION_OUTLINE_WIDTH + 8
    w = min(tw + 2 * pad, video_width - 20)
    h = th + 2 * pad

    img = Image.new("RGBA", (int(w), int(h)), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    # Draw text outline (black) by drawing the text offset in each direction
    outline = CAPTION_OUTLINE_WIDTH
    for dx in range(-outline, outline + 1):
        for dy in range(-outline, outline + 1):
            if dx != 0 or dy != 0:
                draw.multiline_text((pad + dx, pad + dy), line_text, font=font, fill=(0, 0, 0, 255), align="center")

    # Draw the main white text on top
    draw.multiline_text((pad, pad), line_text, font=font, fill=(255, 255, 255, 255), align="center")

    return np.array(img)


def _caption_clip(sentence_text, start_sec, end_sec, video_width, video_height):
    """Build a single caption as an ImageClip placed at center of video."""
    arr = _make_caption_image(sentence_text, video_width)
    if arr is None:
        return None
    clip = ImageClip(arr).set_duration(end_sec - start_sec).set_start(start_sec)
    # Center horizontally and vertically
    x = (video_width - clip.w) // 2
    y = (video_height - clip.h) // 2
    clip = clip.set_pos((x, y))
    return clip
...
def overlay_images_on_video(video_path, image_folder, output_path, text_path, timing_info):
    ...
    try:
        ...
        while i < min_line:
            ...
            # If we found timing information for the sentence (store text + start/end for captions)
            if first_timing_info and last_timing_info:
                start_time = first_timing_info[1]
                end_time = last_timing_info[2]
                finalised_timing.append((cur_sentence, start_time.strftime(format_str), end_time.strftime(format_str)))
            elif cur_sentence:
        ...
        while i < len(finalised_timing):
            _sentence, start_str, end_str = finalised_timing[i]
            if v >= len(image_files):
                v = 0
            image_path = os.path.join(image_folder, image_files[v])
            image_clip = ImageClip(image_path)
            start_time_dt = datetime.strptime(start_str, format_str)
            end_time_dt = datetime.strptime(end_str, format_str)
        ...
        # Add caption clips (on top of image overlays)
        zero = datetime.strptime("0:00:00.00", format_str)
        for sentence_text, start_str, end_str in finalised_timing:
            start_sec = (datetime.strptime(start_str, format_str) - zero).total_seconds()
            end_sec = (datetime.strptime(end_str, format_str) - zero).total_seconds()
            cap = _caption_clip(sentence_text, start_sec, end_sec, video_width, video_height)
            if cap is not None:
                clips.append(cap)
